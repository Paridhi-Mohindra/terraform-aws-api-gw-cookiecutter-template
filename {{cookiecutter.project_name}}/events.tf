resource "aws_api_gateway_resource" "events_resource" {
  ...
  path_part   = "events-service"
}

resource "aws_api_gateway_resource" "events_resource_proxy" {
  rest_api_id = aws_api_gateway_rest_api.main.id
...
resource "aws_api_gateway_method" "events_method" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
  resource_id   = aws_api_gateway_resource.events_resource_proxy.id
  http_method   = "ANY"
  authorization = "NONE"
  request_parameters = {
...
resource "aws_api_gateway_integration" "events_integration" {
  ...
  type                    = "HTTP_PROXY"
  uri                     = "https://virtserver.swaggerhub.com/akashchandwani7/obs-events/v3.1.6/{proxy}"
